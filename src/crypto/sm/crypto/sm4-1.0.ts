...
SM4.prototype = {
  expandKey:function(key){
    ...
    const mk = byteArrayToIntArray(key);
    k[0] = mk[0] ^ this.fk[0];
    k[1] = mk[1] ^ this.fk[1];
    k[2] = mk[2] ^ this.fk[2];
    k[3] = mk[3] ^ this.fk[3];
  ...
  one_encrypt:function(rk, data){
    ...
    const tmpx = new Array(4);
    for(var i = 35;i >= 32;i--){
      tmpx[35-i] = x[i];
    }
    const xbyte = intArrayToByteArray(tmpx);
  ...
  T0:function(ta){
    ...
    b[3] = this.sbox[a[3] & 0xFF];
    const bint = byteToInt(b,0);
    const c = bint ^ (bint<<2|(bint>>>(32-2))) ^ (bint<<10|(bint>>>(32-10))) ^ (bint<<18|(bint>>>(32-18))) ^ (bint<<24|(bint>>>(32-24)));
    return c;
  },
  ...
  encrypt_ecb:function(key,data){
    ...
    const blockLen = 16;
    const loop = parseInt(data.length/blockLen);//注意不能整除会有小数，要取整
    const cipher = new Array((loop+1)*blockLen);
    const tmp = new Array(blockLen);
    let oneCipher = null;
    ...
    const lessData = new Array(data.length%blockLen);
    if(lessData.length > 0){
      arrayCopy(data,loop*blockLen,lessData,0,data.length%blockLen);
    }
    const padding = this.pkcs7padding(lessData,1);
  ...
  decrypt_ecb:function(key,data){
    ...
    const blockLen = 16;
    const loop = data.length/blockLen - 1;
    const tmp = new Array(blockLen);
    let onePlain = null;
    let plain = null;
    //先解密最后一部分，确定数据长度
    ...
    const lastPart = this.pkcs7padding(onePlain,0);
  ...
  decrypt_cbc:function(key,iv,data){
    ...
    const blockLen = 16;
    const loop = data.length/blockLen;
    const tmp = new Array(blockLen);
    let onePlain = null;
    let plain = null;
    ...
    const realPlain = new Array(plain.length-blockLen+lastPart.length);
