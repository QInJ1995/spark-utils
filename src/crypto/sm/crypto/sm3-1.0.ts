...
function SM3(){
  ...
  this.ivByte = [ 0x73,  0x80, 0x16, 0x6f, 0x49,
...
SM3.prototype = {
  ...
  digest:function(data) {
    ...
    const padding = this.padding(data);
    const paddingInt = byteArrayToIntArray(padding);
    this.iterate(paddingInt);
    const macInt = this.vbuf;
  ...
  update:function(data,pos,len) {
    ...
    const loop = parseInt((len+this.dataBufLen)/64);//向下取整
    this.totalLen += len;
  ...
  doFinal:function() {
    let mac;
    const finalData = new Array(this.dataBufLen);

    arrayCopy(this.dataBuf, 0, finalData, 0, this.dataBufLen);
    //对不足64字节的数据进行填充
    const paddingArr = this.padding(finalData);
    const paddingInt = byteArrayToIntArray(paddingArr);
    this.iterate(paddingInt);
    const macInt = this.vbuf;
    mac = intArrayToByteArray(macInt);
    return mac;
