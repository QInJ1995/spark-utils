...
export function rsaEncrypt(data, pubKey, algName) {
  try {
    ...
    const enc = KJUR.crypto.Cipher.encrypt(data, pub, setAlgName);
    return hextob64(enc);
  }
  catch (e) {
    return false;
...
export function rsaDecrypt(data, privKey, algName) {
  try {
    ...
    const value = b64tohex(data);
    const prv = KEYUTIL.getKey(privKey);
    const setAlgName = algName !== null && algName !== void 0 ? algName : "RSAOAEP";
    const dec = KJUR.crypto.Cipher.decrypt(value, prv, setAlgName);
    return dec;
  }
  catch (e) {
    return false;
...
export function rsaSign(data, privKey) {
  try {
    privKey = b64toutf8(privKey);
    const signature = new KJUR.crypto.Signature({ alg: "SHA1withRSA", prvkeypem: privKey, });
...
export function rsaVerify(data, signStr, pubKey) {
  try {
    pubKey = b64toutf8(pubKey);
    const signatureVf = new KJUR.crypto.Signature({ alg: "SHA1withRSA", prvkeypem: pubKey, });
