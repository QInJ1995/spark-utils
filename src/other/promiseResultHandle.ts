...
async function promiseResultHandle(options = {}) {
  ...
  const resultKey =
    options.resultKey || setupDefaults.promiseResultConfig?.resultKey || "data";
  const verifyConfig =
    options.verifyConfig ||
    setupDefaults.promiseResultConfig?.verifyConfig ||
    {};
  ...
  try {
    ...
    if (
      Object.entries(verifyConfig).reduce((pre, cur) => {
      ...
      return Promise.resolve(
        resultKey.split(".").reduce((pre, cur) => {
          pre = pre[cur];
          return pre;
        }, result),
      );
      ...
      return Promise.reject(
        new Error("[spark-utils][promiseResultHandle]: 结果校验不通过!"),
      );
    }
  } catch (error) {
    rejectFn && rejectFn();
    return Promise.reject(
      new Error("[spark-utils][promiseResultHandle]: 结果获取错误!"),
    );
  }
}
