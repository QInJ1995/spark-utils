...
import helperGetUTCDateTime from '../helpers/helperGetUTCDateTime'
...
function parseStringDate (str, format) {
  let arr, sIndex, fIndex, fLen, fItem, rules, rIndex, rLen, tempMatch
  const dates = [0, 0, 1, 0, 0, 0, 0]
...
function toStringDate (str, format) {
  ...
  if (str) {
    ...
    if (isDType || (!format && /^[0-9]{11,15}$/.test(str))) {
      ...
      let tempMatch
      const dates = parseStringDate(str, format || setupDefaults.formatDate)
      const zStr = dates[7]
      if (dates[0]) {
        // 解析时区
        if (zStr) {
