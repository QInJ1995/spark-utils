 ...
 *
 * 旧版经 helperCreatePickOmit(1, 0) 工厂生成，此处内联为命中即保留：
 * - 第二参为函数时按 (value, key, obj) 调用做筛选（this 取调用时的上下文）；
 * - 否则把从第二参起的所有参数（数组展平）作为键名列表，命中即保留；
 * - falsy 入参返回 {}（不匹配的键被丢弃）。
 */
 ...
 * @returns 过滤后的新对象
 */
export function pick(this: unknown, obj: unknown, ...selector: unknown[]): Record<string, unknown> {
